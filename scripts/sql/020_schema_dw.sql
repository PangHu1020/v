-- Business data warehouse schema (ported from legacy MySQL ``dw.sql``).
--
-- Star schema for the e-commerce business the agent serves: 4 dimensions
-- (region, customer, product, date) plus fact_order. Seeded with sample data
-- spanning 2025 Q1 (6 + 20 + 15 + 90 + 100 rows). Foreign key constraints are
-- added at the bottom; legacy MySQL had no explicit FKs so we declare them
-- explicitly here for query-planner correctness.
...
INSERT INTO dw.dim_region (region_id, province, region_name, country)
...
INSERT INTO dw.dim_customer (customer_id, customer_name, gender, member_level)
VALUES ('C001', '李伟', '男', '黄金'),
       ('C002', '王芳', '女', '白银'),
       ('C003', '张敏', '女', '黄金'),
       ('C004', '刘洋', '男', '青铜'),
...
INSERT INTO dw.dim_product (product_id, product_name, category, brand)
...
INSERT INTO dw.dim_date (date_id, year, quarter, month, day)
...
INSERT INTO dw.fact_order (order_id, customer_id, product_id, date_id, region_id, order_quantity, order_amount)
...
-- Explicit FK constraints (legacy MySQL had implicit naming-only references).
ALTER TABLE dw.fact_order
...
ALTER TABLE dw.fact_order
    ADD CONSTRAINT fk_order_region
