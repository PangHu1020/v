...
- **Core Logic (`/backend/v/`)**:
  ...
  - `/rag`: Retrieval layer. `retriever.py` runs one hybrid pass (dense embedding + BM25, Milvus `WeightedRanker`) then reranks the candidate pool via `rerank.py` (`LocalReranker` sidecar / `RemoteReranker` API), truncating to top_k. No cascade / confidence gate / LLM query-rewrite — the query is already agent-written, so a second LLM rewrite was redundant; rerank does the quality lifting. Tools import from here — no logic duplication. See `backend/eval/README.md`.
  - `/mcp`: Model Context Protocol client. Connects to external MCP servers (stdio / streamable-HTTP / SSE) via `langchain-mcp-adapters`' `MultiServerMCPClient`, which discovers + converts every server's tools to LangChain tools in one flat list (`get_tools()`) with built-in `{server}_{tool}` prefixing. `config.py` (server schema + `to_connection()` mapping to the adapters connection dict), `oauth.py` (OAuth 2.0 client_credentials provider — cached + auto-refreshed — plus `ClientCredentialsAuth`, an `httpx.Auth` adapter passed as the connection's `auth`), `cache.py` (L1 in-mem + L2 Redis tool-result cache; write-class tools opt out), `registry.py` (builds the client + a `CachingInterceptor` that layers caching/write-opt-out onto the adapter conversion without manual `StructuredTool` wrapping). The server list lives in `.agent/config.json` (loaded by `configs/agent_config.py`; each entry is `enable`-gated, secrets via `${VAR}` env interpolation); MCP is disabled when no server has `enable:true`. Note: `get_tools()` opens a fresh session per tool call (fine for HTTP; heavier for stdio).
