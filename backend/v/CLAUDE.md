...
- **Core Logic (`/backend/v/`)**:
  ...
  - `/mcp`: Model Context Protocol client. Connects to external MCP servers (stdio / streamable-HTTP / SSE) via `langchain-mcp-adapters`' `MultiServerMCPClient`, which discovers + converts every server's tools to LangChain tools in one flat list (`get_tools()`) with built-in `{server}_{tool}` prefixing. `config.py` (server schema + JSON parser + `to_connection()` mapping to the adapters connection dict), `oauth.py` (OAuth 2.0 client_credentials provider — cached + auto-refreshed — plus `ClientCredentialsAuth`, an `httpx.Auth` adapter passed as the connection's `auth`), `cache.py` (L1 in-mem + L2 Redis tool-result cache; write-class tools opt out), `registry.py` (builds the client + a `CachingInterceptor` that layers caching/write-opt-out onto the adapter conversion without manual `StructuredTool` wrapping). Disabled when `MCP_SERVERS_JSON` is empty. Note: `get_tools()` opens a fresh session per tool call (fine for HTTP; heavier for stdio).
